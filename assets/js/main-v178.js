...
function setupChat(){
  ...
  if(btn && panel && btn.dataset.chatReady !== 'true'){
    btn.dataset.chatReady = 'true';
  ...
  document.addEventListener('click', function(event){
    const clickedToggle = event.target.closest && event.target.closest('#chatToggle');
    const clickedWidget = event.target.closest && event.target.closest('#contactWidget');

    if(clickedToggle){
      event.preventDefault();
      const currentPanel = document.getElementById('chatPanel');
      if(currentPanel) currentPanel.classList.toggle('active');
      return;
    }
