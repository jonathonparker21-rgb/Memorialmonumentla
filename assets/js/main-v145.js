...
/* FORCE_REFRESH_BUILD v1.4.8 2026-04-29 13:10:31 */
const BUILD_VERSION = 'v1.4.8';
...
async function loadBundledContent(){
  const res = await fetch('site-content.json?v=v1.4.8', { cache: 'no-store' });
...
async function loadContent(){
  ...
  try {
    const apiRes = await fetch('/api/get-content?build=v1.4.8', { cache: 'no-store' });
    if (apiRes.ok) {
...
function renderHomeTestimonials(data){
  ...
  if(!track) return;
  const approvedTestimonials = ((data.testimonials || []).filter(t => (t.status || 'approved') === 'approved')); const items = approvedTestimonials.length ? approvedTestimonials : FALLBACK_TESTIMONIALS;
...
function galleryCard(item){
  const beforeImages = (item.beforeImages && item.beforeImages.length ? item.beforeImages : (item.beforeImage ? [item.beforeImage] : []));
  const afterImages = (item.afterImages && item.afterImages.length ? item.afterImages : (item.image ? [item.image] : []));
  ...
  if(beforeImages.length && afterImages.length){
    ...
    const firstBefore = beforeImages[0] || '';
    const firstAfter = afterImages[0] || afterImages[afterImages.length - 1] || '';
    const thumbs = Array.from({ length: pairCount }).map((_, i) => {
      const b = beforeImages[Math.min(i, beforeImages.length - 1)] || '';
      const a = afterImages[Math.min(i, afterImages.length - 1)] || '';
      return `<button class="ba-thumb ${i===0 ? 'active' : ''}" type="button" data-ba-index="${i}" data-before="${b}" data-after="${a}" aria-label="Show comparison photo ${i+1}">
        <img src="${a || b}" alt="${item.title} thumbnail ${i+1}" loading="lazy">
      </button>`;
    }).join('');

    return `<article class="restoration-card restoration-card-ba">
      <div class="ba-wrap pro-ba-wrap" data-ba-wrap>
        <img class="ba-img ba-after" src="${firstAfter}" alt="${item.title} after" loading="lazy">
        <div class="ba-before-layer" style="width:50%;">
          <img class="ba-img ba-before" src="${firstBefore}" alt="${item.title} before" loading="lazy">
        </div>
      ...
      ${pairCount > 1 ? `<div class="ba-thumbs">${thumbs}</div>` : ``}
  ...
  const singleImg = afterImages[0];
  return `<article class="restoration-card restoration-card-single">
    <div class="single-restoration-media">
      <img src="${singleImg}" alt="${item.title}" loading="lazy" onerror="this.closest('.restoration-card').remove();">
...
function setupBeforeAfterSliders(){
  ...
  document.querySelectorAll('.ba-thumbs').forEach(group => {
    if(group.dataset.ready === 'true') return;
    group.dataset.ready = 'true';
    group.querySelectorAll('.ba-thumb').forEach(btn => {
      btn.addEventListener('click', () => {
        const card = btn.closest('.restoration-card');
        const beforeImg = card.querySelector('.ba-before');
        const afterImg = card.querySelector('.ba-after');
        if(beforeImg) beforeImg.src = btn.dataset.before || beforeImg.src;
        if(afterImg) afterImg.src = btn.dataset.after || afterImg.src;
        group.querySelectorAll('.ba-thumb').forEach(b => b.classList.remove('active'));
