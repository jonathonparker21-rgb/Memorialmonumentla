...
/* FORCE_REFRESH_BUILD v1.7.4 2026-04-29 13:10:31 */
...
async function loadBundledContent(){
  const res = await fetch('site-content.json?v=v1.7.4', { cache: 'no-store' });
...
async function loadContent(){
  ...
  try {
    const apiRes = await fetch('/api/get-content?build=v1.7.4&t=' + Date.now(), { cache: 'no-store' });
    if (apiRes.ok) {
