...
/* FORCE_REFRESH_BUILD v1.7.8 2026-04-29 13:10:31 */
const BUILD_VERSION = 'v1.7.8';
...
async function loadBundledContent(){
  const res = await fetch('site-content.json?v=v1.7.8', { cache: 'no-store' });
...
async function loadContent(){
  ...
  try {
    const apiRes = await fetch('/api/get-content?build=v1.7.8&t=' + Date.now(), { cache: 'no-store' });
    if (apiRes.ok) {
