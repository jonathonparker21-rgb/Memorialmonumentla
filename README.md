...
The public site now loads production content from Cloudflare first, but it will not let older saved KV content such as v1.5.6 override this newer build. If the footer still shows an old version, clear the browser site data or delete/update the `site-content` key in the `SITE_CONTENT` KV namespace.
...
## Clean rebase in v1.5.6
- Rebased back to the cleaner v1.5.6 structure.
- Removed the later admin auth/loader experiments by starting over from that base.
- This is intended as a stable checkpoint before rebuilding features again.


## Rebuilt in v1.5.6
- Rebuilt the testimonial system on top of the clean v1.5.6 base.
- Sample testimonials now show in the admin active list.
- Test submissions from the same browser now appear in the admin pending list.


## Fixed in v1.5.6
- Fixed broken repeated script query strings in the admin and public pages.
- This restores the admin JavaScript so the testimonial section can actually run.


## Fixed in v1.5.6
- Testimonials were loading but not rendering in admin.
- fillForm now explicitly calls renderTestimonialsAdmin().
- Seeded sample testimonials and local pending testimonials are both loaded into the admin view.


## New in v1.5.6
- Made the admin testimonial section more compact and closer in feel to the photo upload area.
- Changed the admin page title/header to “Owner Control Panel”.
