...
The public site now loads production content from Cloudflare first, but it will not let older saved KV content such as v1.4.6 override this newer build. If the footer still shows an old version, clear the browser site data or delete/update the `site-content` key in the `SITE_CONTENT` KV namespace.
