...
The public site now loads production content from Cloudflare first, but it will not let older saved KV content such as v1.5.0 override this newer build. If the footer still shows an old version, clear the browser site data or delete/update the `site-content` key in the `SITE_CONTENT` KV namespace.
...
## Fixed in v1.5.0
- Sample testimonials are now force-merged into the admin active testimonials list.
- This works even when older saved content would otherwise hide them.


## Fixed in v1.5.0
- Hard-seeded built-in sample testimonials into the active admin testimonials list.
- Sample testimonials are also written into site-content.json so they persist in the current build.


## Fixed in v1.5.0
- Admin login no longer depends on old browser-saved credentials.
- The credentials you type into the login form are now stored locally and used for admin saves/uploads.
- Use the same username/password you set in Cloudflare Pages Variables.


## Fixed in v1.5.0
- Removed the fragile admin login gate.
- Admin editor now opens directly.
- Saves/uploads/removals use the owner username and password entered in the Admin Access box.


## Fixed in v1.5.0
- Admin now opens directly but still loads the current saved website content.
- Admin loader first checks Cloudflare content, then falls back to bundled site defaults.
- Admin Access credentials are only used for save/upload/remove actions.
